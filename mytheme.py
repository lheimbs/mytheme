...
def backup_file(file):
    ...
    existing_files = sorted(
        [
            f
            for f in os.listdir(file_backup_path)
...
def get_image(image_path, orientation, scale=False):
    ...
    if os.path.isdir(image_path):
        ...
        if not images_in_dir:
            sys.exit(f"ERROR: No Images found in path '{image_path}'.")
...
def set_kitty_colors(colors, kitty_path):
    ...
    COLOR_REGEX = re.compile(
        r"^color(?P<color_num>\d\d?)\s(?P<color>#([0-9a-f]{6}|[0-9a-f]{3}))"
    )
    new_lines = []
    for line in lines:
        color_match = COLOR_REGEX.match(line)
        if color_match:
            color_num = int(color_match.group("color_num"))
            color_rgb = colors[color_num if color_num <= 7 else color_num - 8][
                0 if color_num <= 7 else 1
            ]
...
def reload_polybar():
    ...
    if ret_val.returncode == 0:
        ...
        return True
    logger.error("Failed reloading polybar.")
    return False
...
@click.option(
    ...
    "--no-scaling",
    is_flag=True,
    help="Only consider images that fit the monitor size.",
)
@click.option(
    "-p",
    "--polybar-reload",
    ...
    help=(
        ...
        "CAREFUL: this makes python scripts started in polybar use this "
...
@optgroup.option(
    ...
    type=int,
    default=8,
    help="number of colors to generate (excluding bold).",
)
@optgroup.option(
    "--colorz-minv",
    type=click.IntRange(min=0, max=255),
    default=170,
    help="minimum value for the colors. Default: 170",
)
@optgroup.option(
    "--colorz-maxv",
    type=click.IntRange(min=0, max=255),
    default=200,
    help="maximum value for the colors. Default: 200",
)
@optgroup.option(
    "--colorz-bold",
    type=int,
    default=50,
    help="how much value to add for bold colors. Default: 50",
)
def main(
    image_path,
